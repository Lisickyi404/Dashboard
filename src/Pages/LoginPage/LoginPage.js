...
function LoginPage({setShowLoginPage}){
    ...
    const handleChange = (event) => {
      ...
      const handleSubmit = (event) => {
        ...
        fetch(`https://6317f2c8f6b281877c5feabe.mockapi.io/1/${Math.floor(Math.random() * (10 - 1 + 1) + 1)}`)
        .then((res)=>res.json())
        .then((res)=>dispatch(setData(res)))
        .then(()=>(setShowLoginPage(false)))
    ...
    return(
        <div className="LoginPage_wrapper" >
          ...
          <div className="overlay" onClick={()=>setShowLoginPage(false)}></div>
        </div>
